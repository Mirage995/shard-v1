    ...
    async def run(self, ctx: "StudyContext") -> None:
        ...
        while _attempt <= _MAX_REWRITES:
            ...
            _calib_append({
                ...
                "minimum_experiment": hypothesis.get("minimum_experiment", ""),
...
class ExperimentSandboxPhase(BasePhase):
    ...
    Loop: up to 3 attempts. Each attempt: sandbox N=3 → antagonist review.
      VALID        → proceed to ExperimentValidatePhase
      INVALID_FATAL → set ctx.experiment_status = INCONCLUSIVE, return
      INVALID_CORRECTABLE (attempt < 3) → retry with corrected code
      INVALID_CORRECTABLE (attempt == 3) → set INCONCLUSIVE, return

    On success: writes ctx.experiment_result with replication + antagonist metadata.
    """
    name  = "EXPERIMENT_SANDBOX"
    fatal = False

    async def _run_sandbox_n3(
        self,
        runner,
        code: str,
        ctx: "StudyContext",
        ...
        """Run the same experiment code 3 times independently. Returns replication dict."""
        import re as _re
        ...
        for _rep in range(3):
            ...
            _run_result = await runner.run(
                topic=f"[EXPERIMENT attempt={attempt}] {ctx.topic}",
                code=code,
            ...
            if _m and _run_result.get("success", False):
                ...
                print(f"[REPLICATION] Attempt {attempt}/3 run {_rep+1}/3 — RESULT={_val} ({_elapsed}s)")
            else:
                print(f"[REPLICATION] Attempt {attempt}/3 run {_rep+1}/3 — FAILED ({_elapsed}s)")
        ...
        if _n >= 2:
            ...
            print(
                f"[REPLICATION] Attempt {attempt}/3 N={_n} — "
                f"mean={_rep_mean:.3f} std={_rep_std:.3f} times={_run_times}"
            )
        else:
            _rep_mean = None
            _rep_std  = None
            print(f"[REPLICATION] Attempt {attempt}/3 — only {_n}/3 runs valid")
        ...
        ctx.experiment_result = {
            "success":     _last_result.get("success", False) if _last_result else False,
            "stdout":      _last_result.get("stdout", "")     if _last_result else "",
            "stderr":      _last_result.get("stderr", "")     if _last_result else "",
            "exit_code":   0 if (_last_result and _last_result.get("success")) else 1,
    ...
    async def run(self, ctx: "StudyContext") -> None:
        ...
        # ── Setup runner ──────────────────────────────────────────────────────
        await ctx.emit("EXPERIMENT_SANDBOX", 0, "Running experiment (N=3 replication + Antagonist)...")
        try:
            import os as _os
            ...
            runner = DockerSandboxRunner(sandbox_dir=_sandbox_dir, analysis_fn=None)
        except Exception as exc:
            logger.error("[EXPERIMENT_SANDBOX] Runner init failed: %s", exc)
            ctx.experiment_result = {
                "success": False, "stdout": "", "stderr": str(exc), "exit_code": -1,
                "replication": {"results": [], "mean": None, "std": None, "n": 0, "run_times": []},
            }
            await ctx.emit("EXPERIMENT_SANDBOX", 0, f"Runner init error: {exc}")
            return

        # ── Corrective Antagonist loop (max 3 attempts) ───────────────────────
        hypothesis   = (ctx.structured or {}).get("hypothesis", {})
        current_code = ctx.experiment_code
        max_attempts = 3

        for attempt in range(1, max_attempts + 1):
            # Step 1: N=3 sandbox runs
            try:
                replication = await self._run_sandbox_n3(runner, current_code, ctx, attempt)
            except Exception as exc:
                logger.error("[EXPERIMENT_SANDBOX] N=3 run failed at attempt %d: %s", attempt, exc)
                ctx.experiment_result = {
                    "success": False, "stdout": "", "stderr": str(exc), "exit_code": -1,
                    "replication": {"results": [], "mean": None, "std": None, "n": 0, "run_times": []},
                    "antagonist_verdict": "INVALID_FATAL",
                    "antagonist_reason":  f"Sandbox error: {exc}",
                    "antagonist_attempts": attempt,
                }
                ctx.experiment_status = "INCONCLUSIVE"
                return

            # Step 2: Antagonist review
            try:
                antagonist = await ctx.agent._antagonist_review(
                    hypothesis=hypothesis,
                    experiment_code=current_code,
                    experiment_stdout=ctx.experiment_result.get("stdout", ""),
                    replication_data=replication,
                    attempt=attempt,
                )
            except Exception as exc:
            ...
            logger.info("[ANTAGONIST] Attempt %d/%d — %s: %s", attempt, max_attempts, verdict, reason[:100])
            ...
            if verdict == "VALID":
                ...
                await ctx.emit(
                    ...
                    f"Antagonist VALID at attempt {attempt}/{max_attempts} — "
                    f"N={replication['n']} mean={replication['mean']} std={replication['std']}"
                )
                return

            elif verdict == "INVALID_FATAL":
                logger.warning("[ANTAGONIST] FATAL at attempt %d: %s", attempt, reason)
                ctx.experiment_result.update({
                    "antagonist_verdict":  "INVALID_FATAL",
                    "antagonist_reason":   reason,
                    "antagonist_attempts": attempt,
                })
                ctx.experiment_status = "INCONCLUSIVE"
                await ctx.emit("EXPERIMENT_SANDBOX", 0, f"Antagonist FATAL: {reason[:80]}")
                return

            elif verdict == "INVALID_CORRECTABLE":
                if attempt >= max_attempts:
                    logger.warning(
                        "[ANTAGONIST] CORRECTABLE but max attempts reached at %d: %s",
                        attempt, reason,
                    )
                    ctx.experiment_result.update({
                        "antagonist_verdict":  "INVALID_CORRECTABLE",
                        "antagonist_reason":   reason,
                        "antagonist_attempts": attempt,
                    })
                    ctx.experiment_status = "INCONCLUSIVE"
                    await ctx.emit("EXPERIMENT_SANDBOX", 0, f"Antagonist max attempts: {reason[:80]}")
                    return
                logger.info("[ANTAGONIST] CORRECTABLE at attempt %d, retrying: %s", attempt, reason)
                current_code = antagonist["corrected_code"]
                await ctx.emit(
                    "EXPERIMENT_SANDBOX", 0,
                    f"Antagonist CORRECTABLE at attempt {attempt}/{max_attempts} — retrying..."
                )
                continue

            else:
                logger.error("[ANTAGONIST] Unknown verdict '%s' at attempt %d — INCONCLUSIVE", verdict, attempt)
                ctx.experiment_result.update({
                    "antagonist_verdict":  verdict,
                    "antagonist_reason":   "Unknown verdict",
                    "antagonist_attempts": attempt,
                })
                ctx.experiment_status = "INCONCLUSIVE"
                return

        # Safety net: exhausted attempts without explicit return above
        ctx.experiment_status = "INCONCLUSIVE"
...
class ExperimentValidatePhase(BasePhase):
    ...
    async def run(self, ctx: "StudyContext") -> None:
        ...
        # If ExperimentSandboxPhase already forced INCONCLUSIVE (FATAL or max attempts) → skip validate
        if ctx.experiment_status == "INCONCLUSIVE":
            print("[EXPERIMENT_VALIDATE] SKIPPED — already INCONCLUSIVE from antagonist")
            return
