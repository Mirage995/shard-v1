...
-- ─── RESEARCH HYPOTHESES (Experiment Engine) ────────────────────────────────
-- Hypotheses generated by SYNTHESIZE in research_mode=True.
-- Lifecycle: PENDING -> (CONFIRMED | REFUTED | INCONCLUSIVE | SKIPPED_TOO_COMPLEX)
-- experiment_result stored as JSON string: {stdout, stderr, exit_code, success}
-- source_papers stored as JSON array: [{title, year, url}]
...
CREATE INDEX IF NOT EXISTS idx_hypotheses_topic
    ON research_hypotheses(topic);
