"""d2_2e_analyze.py -- analyzer for D2.2E retry strategy divergence.

D2.2E keeps the D2.2D micro-coupling unchanged and measures whether retry
...
DOC_REPORT = _ROOT / "docs" / "experiments" / "d2_2e_decision_effect_measurement.md"
...
def load_summary(run_root: Path) -> dict:
    path = run_root / "d2_2e_summary.json"
    if not path.exists():
...
def classify_verdict(records: list[dict], aggregates: dict, comparison: dict, sanity: dict) -> tuple[str, str]:
    ...
    if (
        ...
        return (
            "PASS_STRONG",
            "ARM_ON improves at least two decision-quality metrics including retry hash/material shift, with clear signal provenance.",
        )
    if decision_quality_adv or "higher_prior_strategy_named_rate" in primary_adv:
        return (
            "PASS_WEAK",
            "ARM_ON shows clearer retry-strategy divergence by hash or prior strategy naming, while hard outcomes remain limited.",
        )
    return (
        "FAIL",
...
def render(run_root: Path, summary: dict) -> tuple[str, str, dict]:
    ...
    lines = []
    lines.append("# D2.2E Decision Effect Measurement Report\n")
    ...
    lines.append("Does the existing D2.2D reflection micro-coupling already produce measurable retry-strategy divergence, before strengthening the directive?\n")

    lines.append("## Option Implemented\n")
    lines.append("- Planning choice: Option B - Better Retry Quality Metrics First.")
    lines.append("- The D2.2D reflection directive was not strengthened.")
    lines.append("- The D2.2D micro-coupling behavior is kept unchanged.")
    lines.append("- Added measurement: log-derived retry strategy proxy hashes from observer-cycle `Focus` and `gaps` retry lines.\n")
    ...
    lines.append("- `_WINNER_BIAS` unchanged in D2.2E.")
    ...
    lines.append("- D2.2E decision effect measurement only, not D2.2 full.\n")
    ...
    lines.append("| sequence | rep | arm | recovery_success | retries_count | certification_verdict | final_score | repeated_strategy_count | loop_risk_proxy | retry_hash_available | retry_strategy_hash_changed | material_strategy_shift | repeated_strategy_count_by_hash | micro_coupling_applied | reflection_directive_present |")
    lines.append("|---|---:|---|---|---:|---|---:|---:|---:|---|---|---|---:|---|---|")
    for record in records:
        manifest = record["manifest"]
    ...
    lines.append("- D2.2E is not D2.2 full and cannot support a general performance claim.\n")

    lines.append("## Next Step\n")
    lines.append("- If retry hashes show no ARM_ON divergence, consider D2.2F with a pre-registered stronger reflection directive.")
    lines.append("- If retry hashes show ARM_ON divergence, consider D2.3 or D2.2 full only after improving outcome-level metrics.\n")

    lines.append("## Disciplined Claim Boundary\n")
    lines.append("Allowed if supported:")
    lines.append("")
    lines.append("```text")
    lines.append("Under the D2.2D micro-coupling, ARM_ON shows structured evidence of retry-strategy divergence in the observer cycle.")
    lines.append("```")
    lines.append("")
...
def main() -> None:
    if len(sys.argv) > 1:
        ...
        if run_root is None:
            print(f"[d2_2e_analyze] no runs found under {RUNS_ROOT}")
            sys.exit(2)
    if not run_root.exists():
        print(f"[d2_2e_analyze] run root not found: {run_root}")
        sys.exit(2)

    summary = load_summary(run_root)
    markdown, verdict, _details = render(run_root, summary)
    run_report = run_root / "d2_2e_report.md"
    run_report.write_text(markdown, encoding="utf-8")
    DOC_REPORT.write_text(markdown, encoding="utf-8")
    print(markdown)
    print()
    print(f"[d2_2e_analyze] Run report -> {run_report}")
    print(f"[d2_2e_analyze] Docs report -> {DOC_REPORT}")
