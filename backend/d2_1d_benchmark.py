"""d2_1d_benchmark.py -- D2.1D tensions-bias calibration test.

D2.1D is a separate, pre-registered follow-up to D2.1C. It reruns the
same sequential two-topic protocol after the single experimental patch:

    _WINNER_BIAS["tensions"] = (-0.05, +0.15)

The benchmark does not test operational improvement. It only tests whether
the previously silent stress-dominant winner can propagate non-zero
workspace_bias into the next-cycle MoodEngine samples.
"""
...
def _cache_hashes() -> dict:
    ...
    for topic in TOPIC_SEQUENCE:
        ...
        if not path.exists():
            raise SystemExit(f"[D2.1D] missing cache for {topic!r}: {path}")
...
def _run_one(arm: dict, run_dir: Path, run_id: str, run_index_global: int) -> dict:
    ...
    print(f"\n[{run_id}] starting D2.1D subprocess (arm={arm['name']}, no_l3={arm['no_l3']})...")
    ...
    try:
        ...
        exit_code = -1
        out = exc.stdout or ""
        err = (exc.stderr or "") + f"\n[D2.1D] subprocess TIMEOUT after {SUBPROCESS_TIMEOUT}s"
        timeout_hit = True
...
def main() -> None:
    ...
    print("=" * 70)
    print("D2.1D TENSIONS-BIAS CALIBRATION TEST")
    print("=" * 70)
    print(f"Run root:       {run_root}")
    print(f"Topic sequence: {TOPIC_SEQUENCE}")
    print("Patch under test: _WINNER_BIAS['tensions'] = (-0.05, +0.15)")
    print("=" * 70)

    manifests = []
    aborted = False
    for idx, arm in enumerate(ARMS, start=1):
        run_id = f"d2_1d_{arm['name'].lower()}_{idx:03d}"
        manifest = _run_one(arm, run_root / arm["name"].lower(), run_id, idx)
        manifests.append(manifest)
        if args.abort_on_contam and manifest["contaminated"]:
            print(f"[D2.1D] ABORT: contaminated run ({manifest['abort_reason']})")
            aborted = True
            break
    ...
    summary_path = run_root / "d2_1d_summary.json"
    ...
    print(f"Next: python backend/d2_1d_analyze.py {run_root}")
    print("=" * 70)
