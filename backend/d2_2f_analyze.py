"""d2_2f_analyze.py -- analyzer for D2.2F stronger reflection directive.

D2.2F strengthens the D2.2D reflection directive and measures whether retry
strategy proxy hashes diverge under ARM_ON. The analyzer is intentionally
...
RUNS_ROOT = _ROOT / "shard_workspace" / "d2_2f_runs"
DOC_REPORT = _ROOT / "docs" / "experiments" / "d2_2f_stronger_reflection_directive.md"
...
def load_summary(run_root: Path) -> dict:
    path = run_root / "d2_2f_summary.json"
    if not path.exists():
...
def fallback_retry_strategy_hash_metrics(section: str, observer_found: bool) -> dict:
    if not observer_found:
        return {
            "retry_hash_available": False,
            "retry_hash_method": "log_focus_gap_proxy_sha256_12",
            "retry_hash_limitations": ["observer_section_missing"],
            "retry_plans": [],
    ...
    structured = {
        "retry_hash_available": False,
        "retry_hash_method": "log_focus_gap_proxy_sha256_12",
        "retry_hash_limitations": ["fallback unavailable; prefer structured manifest fields"],
        "retry_plans": [],
        "retry_strategy_hashes": [],
        "retry_strategy_hash_changed": UNAVAILABLE,
        "prior_strategy_named": UNAVAILABLE,
        "material_strategy_shift": UNAVAILABLE,
        "repeated_strategy_count_by_hash": UNAVAILABLE,
        "metrics_source": "log_fallback_unavailable",
    }
    return structured
...
def log_decision_coupling_stats(text: str) -> dict:
    ...
    stronger_directive = [line for line in lines if re.search(r"\bstronger_directive=1\b", line)]
    ...
    return {
        ...
        "stronger_directive_present": bool(stronger_directive),
        "stronger_directive_count": len(stronger_directive),
...
def structured_decision_coupling_stats(manifest: dict, fallback: dict) -> dict:
    ...
    return {
        ...
        "stronger_directive_present": structured.get("stronger_directive_present", False),
        "stronger_directive_count": structured.get("stronger_directive_count", 0),
...
def structured_retry_strategy_hash_stats(manifest: dict, fallback: dict) -> dict:
    structured = manifest.get("retry_strategy_hash_metrics")
    if not isinstance(structured, dict):
        return fallback
    return {
        "retry_hash_available": structured.get("retry_hash_available", False),
        "retry_hash_method": structured.get("retry_hash_method", UNAVAILABLE),
        "retry_hash_limitations": structured.get("retry_hash_limitations", []),
        "retry_plans": structured.get("retry_plans", []),
        "retry_strategy_hashes": structured.get("retry_strategy_hashes", []),
        "retry_strategy_hash_changed": structured.get("retry_strategy_hash_changed", UNAVAILABLE),
        "prior_strategy_named": structured.get("prior_strategy_named", UNAVAILABLE),
        "material_strategy_shift": structured.get("material_strategy_shift", UNAVAILABLE),
        "repeated_strategy_count_by_hash": structured.get("repeated_strategy_count_by_hash", UNAVAILABLE),
        "metrics_source": "structured_manifest",
    }
...
def build_records(run_root: Path, summary: dict) -> list[dict]:
    ...
    for manifest in summary.get("manifests", []):
        ...
        retry_strategy = structured_retry_strategy_hash_stats(
            manifest,
            fallback_retry_strategy_hash_metrics(observer_text, found),
        )
        fallback_provenance = (
...
def aggregate_bucket(bucket: list[dict]) -> dict:
    ...
    retry_strategies = [r["retry_strategy"] for r in bucket]
    cert_ranks = []
    ...
    return {
        ...
        "stronger_directive_present_rate": _rate([d.get("stronger_directive_present") for d in decisions]),
        "stronger_directive_count_total": sum(int(d.get("stronger_directive_count", 0)) for d in decisions),
        ...
        "retry_hash_available_rate": _rate([rs.get("retry_hash_available") for rs in retry_strategies]),
        "retry_strategy_hash_changed_rate": _rate([rs.get("retry_strategy_hash_changed") for rs in retry_strategies]),
        "prior_strategy_named_rate": _rate([rs.get("prior_strategy_named") for rs in retry_strategies]),
        "material_strategy_shift_rate": _rate([rs.get("material_strategy_shift") for rs in retry_strategies]),
        "repeated_strategy_count_by_hash_mean": _mean([rs.get("repeated_strategy_count_by_hash") for rs in retry_strategies]),
...
def compare_aggregates(aggregates: dict) -> dict:
    ...
    primary_comparisons = [
        ("higher_retry_strategy_hash_changed_rate", on.get("retry_strategy_hash_changed_rate"), off.get("retry_strategy_hash_changed_rate"), False),
        ("higher_prior_strategy_named_rate", on.get("prior_strategy_named_rate"), off.get("prior_strategy_named_rate"), False),
        ("higher_material_strategy_shift_rate", on.get("material_strategy_shift_rate"), off.get("material_strategy_shift_rate"), False),
        ("lower_repeated_strategy_count_by_hash", on.get("repeated_strategy_count_by_hash_mean"), off.get("repeated_strategy_count_by_hash_mean"), True),
...
def classify_verdict(records: list[dict], aggregates: dict, comparison: dict, sanity: dict) -> tuple[str, str]:
    ...
    arm_on_stronger_count = sum(1 for r in arm_on if r["decision_coupling"].get("stronger_directive_present") is True)
    if arm_on_stronger_count == 0:
        return "FAIL", "ARM_ON signal is present, but the D2.2F stronger reflection directive was not observed."

    retry_hash_available = all(r["retry_strategy"].get("retry_hash_available") is True for r in records)
    if not retry_hash_available:
        return "INCONCLUSIVE", "Retry strategy hash extraction is unavailable or observer retry boundaries are unclear."
    ...
    decision_quality_adv = [
        label for label in primary_adv
        if label in {
            "higher_retry_strategy_hash_changed_rate",
            "higher_material_strategy_shift_rate",
            "lower_repeated_strategy_count_by_hash",
            "higher_prior_strategy_named_rate",
        }
    ]
    severe_regression = (
        "higher_final_score" not in primary_reg
        and "better_certification_verdict" not in primary_reg
        and "higher_recovery_success_rate" not in primary_reg
    )

    if (
        len(primary_adv) >= 2
        and decision_quality_adv
        and severe_regression
        and signal_provenance_clear
    ):
        return (
            "PASS_STRONG",
            "ARM_ON improves at least two decision-quality metrics including retry hash/material shift, with clear signal provenance and stronger directive application.",
        )
    if decision_quality_adv or "higher_prior_strategy_named_rate" in primary_adv:
        return (
            "PASS_WEAK",
            "ARM_ON shows clearer retry-strategy divergence by hash or prior strategy naming after the stronger directive, while hard outcomes remain limited.",
        )
    return (
        "FAIL",
        "ARM_ON signal and stronger directive are present, but no decision-quality metric changes versus ARM_OFF.",
    )
...
def render(run_root: Path, summary: dict) -> tuple[str, str, dict]:
    ...
    lines = []
    lines.append("# D2.2F Stronger Reflection Directive Report\n")
    ...
    lines.append("Does a stronger but still non-overriding reflection directive produce clearer decision-adjacent retry behavior in ARM_ON?\n")
    ...
    lines.append("- Change: the D2.2D reflection directive was strengthened, but remains a prompt modifier and not an override.")
    lines.append("- Stronger directive: explicitly identify the prior failed strategy, explain why it failed, and produce a materially different recovery plan.\n")
    ...
    lines.append("- `_WINNER_BIAS` unchanged in D2.2F.")
    ...
    lines.append("- D2.2F stronger reflection directive only, not D2.2 full.\n")
    ...
    lines.append("| sequence | rep | arm | recovery_success | retries_count | certification_verdict | final_score | repeated_strategy_count | loop_risk_proxy | retry_hash_available | retry_strategy_hash_changed | material_strategy_shift | repeated_strategy_count_by_hash | micro_coupling_applied | reflection_directive_present | stronger_directive_present |")
    lines.append("|---|---:|---|---|---:|---|---:|---:|---:|---|---|---|---:|---|---|---|")
    for record in records:
        manifest = record["manifest"]
        ...
        lines.append(
            ...
            f"| `{retry_strategy.get('retry_hash_available', MISSING)}` "
            f"| `{retry_strategy.get('retry_strategy_hash_changed', MISSING)}` "
            f"| `{retry_strategy.get('material_strategy_shift', MISSING)}` "
            f"| `{retry_strategy.get('repeated_strategy_count_by_hash', MISSING)}` "
            f"| `{decision.get('micro_coupling_applied', MISSING)}` "
            f"| `{decision.get('reflection_directive_present', MISSING)}` "
            f"| `{decision.get('stronger_directive_present', MISSING)}` |"
        )
    lines.append("")

    lines.append("## Retry Strategy Hash Metrics\n")
    lines.append("| sequence | rep | arm | retry_hash_method | retry_hashes | retry_strategy_hash_changed | material_strategy_shift | repeated_strategy_count_by_hash | limitations |")
    lines.append("|---|---:|---|---|---|---|---|---:|---|")
    for record in records:
        manifest = record["manifest"]
        retry_strategy = record["retry_strategy"]
        limitations = "; ".join(str(x) for x in retry_strategy.get("retry_hash_limitations", []))
        lines.append(
            f"| `{manifest['sequence_id']}` | {manifest['rep']} | `{manifest['arm']}` "
            f"| `{retry_strategy.get('retry_hash_method', MISSING)}` "
            f"| `{retry_strategy.get('retry_strategy_hashes', [])}` "
            f"| `{retry_strategy.get('retry_strategy_hash_changed', MISSING)}` "
            f"| `{retry_strategy.get('material_strategy_shift', MISSING)}` "
            f"| `{retry_strategy.get('repeated_strategy_count_by_hash', MISSING)}` "
            f"| `{limitations}` |"
    ...
    lines.append("| sequence | arm | final_score_mean | retry_strategy_hash_changed_rate | material_strategy_shift_rate | repeated_strategy_count_by_hash_mean | mood_min_mean | real_signal_count |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|")
    for seq, arms in aggregates["by_sequence"].items():
        for arm in ("ARM_OFF", "ARM_ON"):
            ...
            lines.append(
                ...
                f"| `{_format_value(bucket.get('retry_strategy_hash_changed_rate', MISSING))}` "
                f"| `{_format_value(bucket.get('material_strategy_shift_rate', MISSING))}` "
                f"| `{_format_value(bucket.get('repeated_strategy_count_by_hash_mean', MISSING))}` "
    ...
    lines.append("## Retry Strategy Hash Method\n")
    lines.append("- Method: `log_focus_gap_proxy_sha256_12`.")
    lines.append("- Source: observer-cycle retry `Focus` and `gaps` log lines.")
    lines.append("- `retry_strategy_hash_changed` compares adjacent retry proxy hashes within the observer cycle.")
    lines.append("- `material_strategy_shift` is a conservative proxy equal to observed hash divergence.")
    lines.append("- `prior_strategy_named` remains `UNAVAILABLE` unless full retry prompt text is logged.\n")
    ...
    lines.append("- Retry strategy hashes are log-derived from observer retry `Focus` and `gaps`, not full semantic plans.")
    lines.append("- Hash changes indicate proxy text changes, not guaranteed semantic strategy shifts.")
    lines.append("- `prior_strategy_named` is `UNAVAILABLE` unless full retry prompt text is logged.")
    lines.append("- D2.2F is not D2.2 full and cannot support a general performance claim.\n")

    lines.append("## Next Step\n")
    lines.append("- If stronger directive still does not move decision-quality metrics, pause prompt tuning and improve semantic retry-plan instrumentation.")
    lines.append("- If stronger directive moves decision-quality metrics without score/cert regression, consider a small D2.3 operational probe before D2.2 full.\n")

    lines.append("## Disciplined Claim Boundary\n")
    lines.append("Allowed if supported:")
    lines.append("")
    lines.append("```text")
    lines.append("Under a stronger but still non-overriding reflection directive, ARM_ON shows clearer decision-adjacent retry behavior than ARM_OFF under the controlled micro protocol.")
    lines.append("```")
    lines.append("")
...
def main() -> None:
    if len(sys.argv) > 1:
        ...
        if run_root is None:
            print(f"[d2_2f_analyze] no runs found under {RUNS_ROOT}")
            sys.exit(2)
    if not run_root.exists():
        print(f"[d2_2f_analyze] run root not found: {run_root}")
        sys.exit(2)

    summary = load_summary(run_root)
    markdown, verdict, _details = render(run_root, summary)
    run_report = run_root / "d2_2f_report.md"
    run_report.write_text(markdown, encoding="utf-8")
    DOC_REPORT.write_text(markdown, encoding="utf-8")
    print(markdown)
    print()
    print(f"[d2_2f_analyze] Run report -> {run_report}")
    print(f"[d2_2f_analyze] Docs report -> {DOC_REPORT}")
