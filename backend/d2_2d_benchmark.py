"""d2_2d_benchmark.py -- D2.2D micro decision coupling validation.

D2.2D reuses the D2.2A micro protocol after a pre-registered
reflection-directive micro-coupling. It keeps the validated cached-source
harness:
...
only produces clean run artifacts for an analyzer to test whether the
calibrated GWT/Mood signal can influence decision-adjacent reflection or
strategy-shift prompting.
"""
...
PLANNING_COMMIT = "8b10327ad5f2c5e9ffcd75cd14b71e74ca8503ed"
...
def _preflight_cache_hashes() -> dict:
    ...
    if missing:
        lines = ["[D2.2D] missing cached source files; benchmark will not start."]
...
def _run_one(sequence: dict, rep: int, arm: dict, run_dir: Path, run_id: str, run_index_global: int) -> dict:
    ...
    print(
        f"\n[{run_id}] starting D2.2D subprocess "
    ...
    try:
        ...
        exit_code = -1
        out = exc.stdout or ""
        err = (exc.stderr or "") + f"\n[D2.2D] subprocess TIMEOUT after {SUBPROCESS_TIMEOUT}s"
        timeout_hit = True
...
def main() -> None:
    ...
    print("=" * 70)
    print("D2.2D MICRO DECISION COUPLING")
    print("=" * 70)
    ...
    for sequence in TOPIC_SEQUENCES:
        for rep in REPLICAS:
            for arm in ARMS:
                run_index += 1
                run_id = f"d2_2d_{sequence['id']}_rep{rep:02d}_{arm['name'].lower()}"
                ...
                if args.abort_on_contam and (manifest["contaminated"] or manifest["abort_reason"]):
                    print(f"[D2.2D] ABORT: run failed sanity ({manifest['abort_reason']})")
                    aborted = True
                    break
            if aborted:
                break
    ...
    summary_path = run_root / "d2_2d_summary.json"
    ...
    print(f"Next: python backend/d2_2d_analyze.py {run_root}")
    print("=" * 70)
