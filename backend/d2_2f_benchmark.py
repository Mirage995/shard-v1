"""d2_2f_benchmark.py -- D2.2F stronger reflection directive validation.

D2.2F reuses the D2.2E protocol after strengthening the reflection directive
at the same retry/reflection point. It keeps retry strategy hash
instrumentation so the analyzer can test whether retry plans materially
diverge under ARM_ON:
...
PLANNING_COMMIT = "5e6b06907b5c591728af711f3378117838210874"
...
def _count_markers(text: str) -> dict:
    ...
    return {
        ...
        "stronger_directive_count": len(re.findall(r"\[D2_2D_DECISION_COUPLING\].*stronger_directive=1", text)),
...
def _preflight_cache_hashes() -> dict:
    ...
    if missing:
        lines = ["[D2.2F] missing cached source files; benchmark will not start."]
...
def _run_one(sequence: dict, rep: int, arm: dict, run_dir: Path, run_id: str, run_index_global: int) -> dict:
    ...
    print(
        f"\n[{run_id}] starting D2.2F subprocess "
    ...
    try:
        ...
        exit_code = -1
        out = exc.stdout or ""
        err = (exc.stderr or "") + f"\n[D2.2F] subprocess TIMEOUT after {SUBPROCESS_TIMEOUT}s"
        timeout_hit = True
    ...
    manifest = {
        ...
        "micro_decision_coupling": {
            ...
            "directive_strengthened_in_d2_2f": True,
...
def main() -> None:
    ...
    print("=" * 70)
    print("D2.2F STRONGER REFLECTION DIRECTIVE")
    print("=" * 70)
    ...
    for sequence in TOPIC_SEQUENCES:
        for rep in REPLICAS:
            for arm in ARMS:
                run_index += 1
                run_id = f"d2_2f_{sequence['id']}_rep{rep:02d}_{arm['name'].lower()}"
                ...
                if args.abort_on_contam and (manifest["contaminated"] or manifest["abort_reason"]):
                    print(f"[D2.2F] ABORT: run failed sanity ({manifest['abort_reason']})")
                    aborted = True
                    break
            if aborted:
                break
    ...
    summary_path = run_root / "d2_2f_summary.json"
    ...
    print(f"Next: python backend/d2_2f_analyze.py {run_root}")
    print("=" * 70)
