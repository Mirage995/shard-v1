# SHARD V2 - Advanced Design Assistant
...
> **SHARD** = **A**dvanced **D**esign **A**ssistant

SHARD V2 is a sophisticated AI assistant designed for multimodal interaction. It combines Google's Gemini 2.5 Native Audio with computer vision, gesture control, and 3D CAD generation in a Electron desktop application.
...
SHARD's "Minority Report" interface uses your webcam to detect hand gestures:
...
graph TB
    ...
    subgraph Backend ["Backend (Python 3.11 + FastAPI)"]
        ...
        SHARD[SHARD.py<br/>Gemini Live API]
...
To use the secure voice features, SHARD needs to know what you look like.
...
SHARD V2 can slice STL files and send them directly to your 3D printer.
...
SHARD uses **OrcaSlicer** (recommended) or PrusaSlicer to generate G-code.
1. Download and install [OrcaSlicer](https://github.com/SoftFever/OrcaSlicer).
2. Run it once to ensure profiles are created.
3. SHARD automatically detects the installation path.

**Step 2: Connect Printer**
1. Ensure your printer and computer are on the **same Wi-Fi network**.
2. Open the **Printer Window** in SHARD (Cube icon).
3. SHARD automatically scans for printers using mDNS.
...
SHARD uses Google's Gemini API for voice and intelligence. You need a free API key.
...
1. **Voice Check**: Say "Hello SHARD". She should respond.
...
### 🖨️ Printing & Slicing
- **Auto-Discovery**: SHARD automatically finds printers on your network.
- **Slicing**: Click "Slice & Print" on any generated 3D model.
- **Profiles**: SHARD intelligently selects the correct OrcaSlicer profile based on your printer's name (e.g., "Creality K1").
...
**Solution**:
This is a server-side issue from the Gemini API. Simply reconnect by clicking the connect button or saying "Hello SHARD" again. If it persists, check your internet connection or try again later.
...
│   ├── SHARD.py                  # Gemini Live API integration
