...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed_path.path == "/users":
            ...
            new_user = eval(body.decode())
            new_user_id = max(self.users.keys()) + 1
            self.users[new_user_id] = new_user
            self.send_response(201)
            self.send_header("Content-type", "application/json")
            self.end_headers()
            self.wfile.write(str(new_user).encode())
        else:
    ...
    def do_PUT(self):
        ...
        if parsed_path.path.startswith("/users/"):
            ...
            if user_id in self.users:
                content_length = int(self.headers["Content-Length"])
                body = self.rfile.read(content_length)
                updated_user = eval(body.decode())
                self.users[user_id] = updated_user
                self.send_response(200)
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(str(updated_user).encode())
            else:
                self.send_response(404)
                self.send_header("Content-type", "application/json")
                self.end_headers()
